// src/pages/TaskDetailPage.jsx (Refactored from DetailPanel.tsx)
...
export default function TaskDetailPage({ record, onStatusChange }) {
    ...
    return (
      <div className="flex-1 flex flex-col h-full bg-white">
        ...
        <div className="px-7 py-5 border-b border-(--border)">
          <h1 className="text-lg font-semibold tracking-tight">Task Detail Page</h1>
        </div>
        <div className="flex-1 flex flex-col items-center justify-center text-center px-8">
          ...
          <p className="text-xs text-zinc-400 mt-1">The task you are looking for does not exist or was deleted.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex-1 flex flex-col h-full overflow-y-auto bg-white">
      ...
      {/* Meta */}
      <div className="px-7 py-5 grid grid-cols-2 gap-x-6 gap-y-5 border-b border-(--border)">
        <div>
          <p className="text-xs text-zinc-400 uppercase tracking-widest mb-1.5 font-medium">Status</p>
          <StatusBadge status={record.status} />
        </div>
        <div>
          <p className="text-xs text-zinc-400 uppercase tracking-widest mb-1.5 font-medium">Priority</p>
          <PriorityBadge priority={record.priority} />
        </div>
      </div>

      {/* Description */}
      <div className="px-7 py-5 border-b border-(--border) flex-1">
        <p className="text-xs text-zinc-400 uppercase tracking-widest mb-3 font-medium">Description</p>
      ...
      <div className="px-7 py-5">
        <p className="text-xs text-zinc-400 uppercase tracking-widest mb-3 font-medium">Change status</p>
